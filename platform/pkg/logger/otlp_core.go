// OTLP Core Component
//
// Компонент преобразования логов Zap в OpenTelemetry Records
// для отправки централизованному OTel Collector'у через gRPC.
//
...
// SimpleOTLPCore преобразует zap-записи в OpenTelemetry Records
// и отправляет их напрямую в OTLP Collector
...
// mapZapToOtelSeverity преобразует zap level в OTLP Severity
...
// makeBaseRecord собирает базовый record без атрибутов
...
// encodeFieldsToAttrs подготавливает атрибуты из zap-полей.
// Используем zapcore.NewMapObjectEncoder(), чтобы развернуть []zapcore.Field
// в карту. Переносим только базовые типы в OTLP KeyValue.
...
// emitWithTimeout отправляет запись в OTLP с коротким таймаутом
